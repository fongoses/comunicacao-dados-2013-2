function intervalo_fft
    ...
    % Não-Normalizado
    fft1 = abs(fft(x1, 90)); % Cálculo da FFT para o intervalo 1
    fft2 = abs(fft(x2, 90)); % Cálculo da FFT para o intervalo 2
    fft3 = abs(fft(x3, 90)); % Cálculo da FFT para o intervalo 3
    ...
    hold off;
    subplot(3, 2, 2);
    plot(s, fft1, '-x'), title('FFT com 1 Período'), axis([0 1 0 50]), grid on;
    ...
    hold off;
    subplot(3, 2, 4);
    plot(s, fft1, '-x'), title('FFT com 2 Períodos'), axis([0 1 0 50]), grid on;
    ...
    hold off;
    subplot(3, 2, 6);
    plot(s, fft3, '-x'), title('FFT com 3 Períodos'), axis([0 1 0 50]), grid on;
